...
**注意:** 本项目正在重构之中，完成后将会有更好的体验及更高的(TOP10)成功率，并且代码将会在 [tjuptattendance](https://github.com/azureqaq/tjuptattendance/) 开源。届时本仓库将 **归档一段时间后删除**。在这段时间内，如果有问题或对重构版本的建议，非常欢迎 Issue！新版本将会有更简单的使用方式、更灵活的配置、更加自动化、更省心。重构的版本依旧使用 `Rust` 作为主要开发语言，并且辅以 `Python` 来实现部分 配置/自动化 功能。为了实现这些目标，可能还需要较久的时间，本仓库依旧会继续修复影响使用的BUG。本人能力有限，对 *自建数据库API服务* 部分不是很了解，如果有想参与开发的，请新建 Issue，非常感谢!
